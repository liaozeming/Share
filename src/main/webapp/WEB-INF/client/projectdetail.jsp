...
<div class="adbot wrapcenter clearfix">
    <div class="adbot_l fl">
        ...
        <div class="table" style="display:none;"  id="commnetShow"><!--项目评论-->
...
<script>
    ...
    function ShowComments(id) {
        ...
        $.ajax({
            url:'${pageContext.request.contextPath}/ShowComments.action',
            type:'get',
            data: {
                id:id
            },
            ...
            success: function(data){
                if(data!=null){
                    ...
                    $(data).each(function (index) {
                        ...
                        //时间戳转换年月日
                        var date=new Date(comment.starttime)
                        var year = date.getFullYear()+'年';
                        var month = date.getMonth()+1+'月';
                        var date = date.getDate()+'日';
                        var date1=[year,month,date].join('-');
                        ...
                        //把评论的图片插入到各自的id下
                        var a= "#commentImg"+comment.id;

                        var str5='<div class="projectappraise">'+
                                '<p><img src="/file/'+comment.headimg+'" class="absolute projectappraise_head"/></p>'+
                                ' <p><span class="projectappraise_name">'+comment.fname+'</span><span class="projectappraise_time">发布于'+date1+'</span></p>'+
                                '<p class="projectappraise_content">'+comment.content+'</p>'+
                                 '<p id="commentImg'+comment.id+'"></p>';
                        $("#commnetShow").append(str5);

                        //遍历图片数组加入图片
                        $(comment.imageUrls).each(function (index) {
                            // alert(comment.imageUrls[index]);
                            var str6='<span class="projectappraise_img"><img src="/file/'+comment.imageUrls[index]+'"></span>'
                            // alert(imageList[index]);
                            $(a).append(str6);
                        });
